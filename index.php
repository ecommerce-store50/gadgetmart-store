...
function uploadProductImage(): ?string {
    ...
    $filename = time() . '_' . preg_replace('/[^a-zA-Z0-9_.-]/', '_', basename($_FILES['image_file']['name']));
...
if (isset($_GET['action']) && $_GET['action'] === 'edit' && !empty($_GET['id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $uploadedImage = uploadProductImage();
        $image = $uploadedImage ?: trim($_POST['image'] ?? $product['image']);
    ...
    <body class="admin-body">
      ...
      <main class="admin-main">
        <div class="topbar"><h1>প্রোডাক্ট সম্পাদনা</h1></div>
        <section class="panel">
          <?php if ($message): ?><div class="alert success"><?= e($message) ?></div><?php endif; ?>
          <form method="post" enctype="multipart/form-data" class="stacked-form small-form">
            ...
            <label>ছব��� URL<input type="text" name="image" value="<?= e($product['image'] ?? '') ?>"></label>
            <label>ছবি আপলোড<input type="file" name="image_file" accept="image/*"></label>
