...
function cartItems(): array { $cart=$_SESSION['cart']??[]; if(!$cart)return []; $ids=array_keys($cart); $marks=implode(',',array_fill(0,count($ids),'?')); $q=db()->prepare("SELECT * FROM products WHERE id IN ($marks) AND is_active=1"); $q->execute($ids); $items=[]; foreach($q->fetchAll() as $p) $items[]=['product'=>$p,'qty'=>max(1,min((int)$p['stock'],(int)($cart[$p['id']]['qty']??1)))]; return $items; }
...
function updateCartQty(int $id,int $qty): void { $cart=$_SESSION['cart']??[]; if($qty<=0){unset($cart[$id]);}else{$q=db()->prepare('SELECT stock FROM products WHERE id=? AND is_active=1');$q->execute([$id]);$stock=(int)$q->fetchColumn();if($stock>0)$cart[$id]=['qty'=>min($qty,$stock)];else unset($cart[$id]);}$_SESSION['cart']=$cart; }
